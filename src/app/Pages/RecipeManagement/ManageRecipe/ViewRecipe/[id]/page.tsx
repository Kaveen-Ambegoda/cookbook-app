  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await axios.get(`https://localhost:7205/api/Recipe/${id}`);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 md:p-10 font-sans bg-yellow-50">
      <div className="bg-green-200 p-6 rounded-xl mt-10">
        <h1 className="text-3xl font-bold text-center text-green-900 mb-8">{recipe.title}</h1>
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-stretch">
          ...
          <div className="w-full md:w-1/3 flex flex-col justify-center self-center gap-4">
            ...
            <div className="bg-white rounded-lg p-4 border-l-4 border-green-500 shadow">
              ...
              <ul className="text-sm text-gray-700 space-y-1">
                <li><strong>Calories:</strong> {recipe.calories ?? "N/A"}</li>
                <li><strong>Protein:</strong> {recipe.protein ?? "N/A"}g</li>
                <li><strong>Carbohydrates:</strong> {recipe.carbohydrates ?? "N/A"}g</li>
              </ul>
            </div>
          </div>
        </div>
