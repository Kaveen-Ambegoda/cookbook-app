...
import { Users, BookOpen, Cookie, ChefHat, Bell, X, LogOut, Home } from 'lucide-react';
...
export default function Sidebar({ isOpen, closeSidebar }: { isOpen: boolean, closeSidebar: () => void }) {
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-30 w-64 transform shadow-lg transition-transform duration-300 lg:static lg:inset-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
      style={{ backgroundColor: "#FFD476" }}
    >
      <div className="flex items-center justify-between h-16 px-6 border-b">
        <span className="text-xl font-bold text-stone-900">CookBook Admin</span>
        <button onClick={closeSidebar} className="p-1 rounded-md lg:hidden hover:bg-gray-200 focus:outline-none" aria-label="Close Sidebar">
          <X size={20} />
        </button>
      </div>
      <nav className="px-4 py-6">
        ...
        <div className="pt-8 mt-8 border-t">
          <button className="flex items-center w-full px-4 py-3 text-sm text-black rounded-md hover:bg-gray-100 focus:outline-none">
