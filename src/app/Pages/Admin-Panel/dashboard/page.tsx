...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // You can replace this with actual data fetching logic (e.g., API requests)
    setStats([
      {
        id: 'users',
        label: 'Total Users',
  ...
  // Function to simulate adding new activity
        ...
        {stats.map((stat) => (
          <Card key={stat.id} className="p-6 shadow-sm">
            <div className="flex items-center">
              ...
              <div className="ml-5">
                <p className="text-sm font-medium text-yellow-500">{stat.label}</p>
          ...
          <div className="space-y-4">
            {recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-start">
                <div
                  className={`w-2 h-2 mt-2 rounded-full mr-3 ${getActivityColor(activity.type)}`}
                ></div>
                <div>
                  <p className="text-sm">
